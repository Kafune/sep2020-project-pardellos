...
describe("Laterlezer extension e2e tests", () => {
  let theBrowser, thePage, thePage2;
  ...
  beforeAll(async () => {
    ...
    theBrowser2 = await puppeteer.launch({
      headless: false,
      slowMo: 5,
      defaultViewport: null,
      devtools: false,
      args: ["--window-size=700,700", "--window-position=750,0"]   
    });
    thePage2 = await theBrowser2.newPage();
    await thePage2.goto("http://localhost:3000/login");
  });
  ...
  test("User logs in webapplication", async () => {
    ...
    await thePage2.waitForTimeout(1500);
    await thePage2.type("input[id=email]", testEmail);
    await thePage2.type("input[id=password]", testPassword);
    await thePage2.click('a[id="login"]');
  });  
